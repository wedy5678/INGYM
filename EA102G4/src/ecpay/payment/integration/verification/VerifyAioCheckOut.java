...
public class VerifyAioCheckOut extends PaymentVerifyBase {
	...
	public void verifyInvoice(InvoiceObj obj){
		// 1. ÊØîÂ?çÁâπÊÆäÊ?Ñ‰?çÂ?ºÁõ∏‰æùÈ?Ê±?
		// a. [CarruerType]?Ç∫ 1 => CustomerID ‰∏çËÉΩ?Ç∫Á©?
		...
		} else if(!obj.getCustomerID().isEmpty()){ // [CustomerID]‰∏çÁÇ∫Á©? => CarruerType ‰∏çËÉΩ?Ç∫Á©?
			if(obj.getCarruerType().isEmpty())
				throw new EcpayException("CarruerType cannot be empty when CustomerID is not empty.");
		}
		// b. ??óÂç∞Ë®ªË?ò[Print]?Ç∫1 => CustomerName, CustomerAddr
		...
		// c. CustomerPhone??åCustomerEmail?á≥Â∞ë‰??ãÊ?âÂ??
		...
		// d. [TaxType]?Ç∫ 2 => ClearanceMark = 1 or 2
		...
		// e. Áµ±‰?Á∑®Ë?ü[CustomerIdentifier]??âÂ?ºÊ?? => CarruerType != 1 or 2, *Donation = 2, print = 1
		...
		// f. CarruerType??âÂ?ºÊ??=> Print = 0
		if(!obj.getCarruerType().isEmpty())
			if(obj.getPrint().equals("1"))
				throw new EcpayException("Print must be 0 when CarruerType is given.");
		// [CarruerType]?Ç∫'' or 1 ??? => CarruerNum = '', [CarruerType]?Ç∫ 2Ôº? CarruerNum = ?õ∫ÂÆöÈï∑Â∫¶ÁÇ∫ 16 ‰∏îÊ†ºÂºèÁÇ∫ 2 Á¢ºÂ§ßÂ∞èÂØ´Â≠óÊ?çÂ?†‰?? 14 Á¢ºÊï∏Â≠ó„?? [CarruerType]?Ç∫ 3 ÔºåÂ∏∂?õ∫ÂÆöÈï∑Â∫¶ÁÇ∫ 8 ‰∏îÊ†ºÂºèÁÇ∫ 1 Á¢ºÊ?úÁ?ö„??/?çÂ?†‰?äÁî± 7 Á¢ºÊï∏Â≠óÂ?äÂ§ßÂ∞èÂØ´Â≠óÊ?çÁ?ÑÊ??
		...
		// Donation = 1 => LoveCode‰∏çËÉΩ?Ç∫Á©?, print = 0
		...
		// 2. ÊØîÂ?çÂ?ÜÂ?ÅÂ?çÁ®±ÔºåÊï∏??èÔ?åÂñÆ‰ΩçÔ?åÂÉπ?†ºÔºåtax??ÖÁõÆ?ï∏??èÊòØ?ê¶‰∏??á¥
		...
		// ??ÜÂ?ÅÂ?çÁ®±?ê´??âÁÆ°Á∑? => Ë™çÁÇ∫?òØÂ§öÊ®£??ÜÂ?? *InvoiceItemNameÔº? *InvoiceItemCount Ôº?*InvoiceItemWordÔº? *InvoiceItemPrice InvoiceItemTaxType?ê‰??î®ÁÆ°Á?öÂ?ÜÂâ≤ÔºåË?àÁ?óÊï∏??èÂ?åË?áÁ¨¨‰∏??ãÊ?îÂ??
		if(obj.getInvoiceItemName().contains("|")){
			...
			// Ë™≤Á?ÖÈ?ûÂà•[TaxType] = 9 ??? => InvoiceItemTaxType ?ÉΩ?ê´???1,2 3(and at least contains one 1 and other)
			...
			// Ê≤íÊ?âÁÆ°Á∑? => ?ê‰?Ê™¢Êü•Âæ?4??ÖÊ?âÁÑ°ÁÆ°Á??
