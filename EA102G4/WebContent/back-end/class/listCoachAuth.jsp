    ...
    <i class="fas fa-object-ungroup"></i>
        ...
        <div id="three" class="collapse show" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
          <div class="bg-white py-2 collapse-inner rounded">
			...
			<a class="collapse-item active" href="<%=request.getContextPath() %>/back-end/class/listCoachAuth.jsp">教練審查</a>
          </div>
        </div>
      </li>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-4 text-gray-800">教練資格審查</h1>
	            <!-- DataTales Example -->
					    ...
					    <tr>
					      ...
					      <td>
						      	<c:forEach var="classAuthVO" items="${classAuthSvc.all}">
						      		<div>
						      			<c:if test="${proVO.pro_ID == classAuthVO.pro_ID}">
						      			<c:if test="${classAuthVO.ca_status==1}">
						      			${classTypeSvc.getOneClassType(classAuthVO.c_type_no).t_name}
					      ...
					      <td width=150px>
					      <form method="post" action="<%=request.getContextPath()%>/back-end/class/pro.do">
					      
							      <select size="1" name="pro_auth">
							      		<option value=0 ${(proVO.pro_auth==1)?'':'selected'}>否 
							      		<option value=1 ${(proVO.pro_auth==1)?'selected':''}>是 
						      	  </select>
						      	  
						      	  <input type="hidden" name="pro_ID" value="${proVO.pro_ID}">				      	  
						    <input type="hidden" name="action" value="updateProAuth">
