...
<table>
	...
	<c:forEach var="memVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
		...
		<tr>
			...
			<td>${memVO.mem_psw}</td>
			<td><fmt:formatDate value="${memVO.mem_bir}" pattern = "yyyy-MM-dd"/></td>
			<td>${memVO.sex}</td>
			<td>${memVO.mem_addr}</td>
			<td>${memVO.mem_email}</td>
			<td>${memVO.mem_phone}</td>
			<td>${memVO.mem_absent}</td>
			<td>${memVO.coin}</td>
			<td>${memVO.mem_resume}</td>
			<td><fmt:formatDate value="${memVO.m_reg_date}" pattern = "yyyy-MM-dd HH:mm:ss"/></td>
			<td>${memVO.sel_auth}</td> 
			<td>${memVO.art_auth}</td>
			<td>${memVO.com_auth}</td>
			<td>
			...
			<td>
			  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/mem/mem.do" style="margin-bottom: 0px;">
			     ...
			     <input type="hidden" name="mem_id"  value="${memVO.mem_id}">
			     <input type="hidden" name="action"	value="getOne_For_Update_psw"></FORM>
			</td>
			<td>
			  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/mem/mem.do" style="margin-bottom: 0px;">
