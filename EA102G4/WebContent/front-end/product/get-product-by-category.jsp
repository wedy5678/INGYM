...
Set<ProductVO> list = (Set) request.getAttribute("set");
String caten = null;
for (ProductVO p : list) {
	System.out.println(p.getProductNo());
	caten = p.getCategoryNo();
}

CategoryService categoryService2 = new CategoryService();
List<Product_categoryVO> list_category = categoryService2.getAll();
pageContext.setAttribute("list_category", list_category);
// 	Iterator<Product_categoryVO> list_category_it = list_category.iterator();
String catename = null;
try {
	catename = categoryService2.getOneCategory(caten).getCategoryName();

} catch (Exception e) {
	System.out.println("這個類別沒商品");
}
pageContext.setAttribute("catename", catename);
%>
...
<body>
	...
	<section class="shopping-area margin-top-90">
		<div class="container">
					...
					<div class="col-lg-12">
						<div class="row d-flex justify-content-lg-between flex-column flex-md-row align-items-center text-center">
							<div class="form-field margin-top-10 margin-bottom-10 ">
								...
								<!--                                     <option value="dum">Dumbell</option> -->
								<!--                                     <option value="yoga">Yoga ball</option> -->
								<!--                                 </select> -->
							...
							<!-- 商品分類 -->
							<input type="hidden" value="${catename}" id="hi">
							<div class="search-menu" id="search-menu">
								<ul id="cateli">
									<li>
										<a href="${pageContext.request.contextPath}/front-end/product/list-all-product.jsp">All</a>
									</li>
									<c:forEach var="product_categoryVO" items="${list_category}">
										<li>
											<form action="${pageContext.request.contextPath}/product_category/category.do" enctype="multipart/form-data">
												...
												<a href="#" onclick=subCategory(this)>
													...
													<input type="hidden" name="categoryNo" value="${product_categoryVO.categoryNo}" />
												</a>
											</form>
										</li>
									</c:forEach>
			...
			<div class="row shopping-slider-item text-center">
				...
				<c:forEach var="productVO" items="${set}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
						...
						<div class="shop-single-item margin-bottom-50">
							<div id="pHover" class="top-content" style="text-align: -webkit-center;">
								<div class="thumb" style="width: 198px; height: 198px; display: flex; align-items: center; justify-content: center;">
									<img src="data:image/jpg;base64,${photoService.get(productVO.getProductNo()).base64Image}" style="max-width: 198px; max-height: 198px" />
								</div>
							...
							<div class="bottom-content">
								...
								<FORM METHOD="post" action="${pageContext.request.contextPath}/product/product.do" enctype="multipart/form-data" id="fff">
									<h5>
										<a href="#" onclick=sub(this)>${productVO.pName}
											<input type="hidden" name="productNo" value="${productVO.productNo}">
	...
	<script src="${pageContext.request.contextPath}/front-end/product/assets/js/wedy-list-product-by-cate.js"></script>
