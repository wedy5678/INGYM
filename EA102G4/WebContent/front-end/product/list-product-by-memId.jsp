...
<%
	...
	if (memVOLogin == null) {			//if (memVOLogin == null && empVO == null) {
		...
		List<ProductVO> list = productService.findByMemId(memVO.getMem_id());
			...
			<div class="row">
				<div class="col-lg-12">
					<div class="cart-title">
						<h2 style="font-family:Microsoft JhengHei;font-size: 30px;font-weight: 500;">賣家商品管理</h2>
					</div>
				</div>
			</div>
			<div class="row">
...
<!-- 									<button class="confirmModal"> Delete </button> -->
								...
								<c:forEach var="productVO" items="${list}"
									...
									<tr>
										<td style="width:146px;height:146px;">
										...
										<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/product/product.do">
											<p class="font-semibold">
												<a class="pName" href="#" style="color:inherit;" onclick=parent3FormSub(this)>${productVO.pName}
												</a>
											</p>
										...
										<td><p><fmt:formatNumber value="${productVO.pPrice}" pattern="0.#" />元</p></td>
										<td><p><fmt:formatNumber value="${productVO.pStock}" pattern="0.#" /></p></td>
										<td><p>${listProductByMemId[productVO.pStatus]}</p></td>
										
										
										<td>
											<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/product/product.do">
												<input type="submit" class="btn btn-success" value="修改商品" name="updateProductInputBtn">
												<input type="hidden" name="action" value="get-one-product-for-update">
												<input type="hidden" name="productNo" value="${productVO.productNo}">
												<input type="hidden" class="pStatus" value="${productVO.pStatus}">
											</FORM>
										</td>
										<td>
											<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/product/product.do">
												<input type="button" class="btn btn-danger confirmModal" value="下架商品" name="offProductInputBtn" onclick=deleteProduct(this);>
												<input type="hidden" value="下架商品">
												<input type="hidden" name="productNo" value="${productVO.productNo}">
