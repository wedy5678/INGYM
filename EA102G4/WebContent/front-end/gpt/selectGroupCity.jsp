...
<%
	...
	String cityScope = (String) request.getSession().getAttribute("cityScope");
	GroupService groupSvc = new GroupService();
	List<GroupVO> list = groupSvc.getGroupByCity(cityScope);
											...
											<div class="col-lg-${colBox[(listIndex.index%6)]} %>" style="padding-left: 10px; padding-right: 10px;">
												<div class="blog-item-inner">
													<div class="blog-detail">
														<div class="blog-meta">
															<h6>
																<span class="material-icons">book_online</span>&nbsp;<span class="groStart">${groupVO.gro_start}</span> 
																<span class="float-right text-right"> 
																	<span class="material-icons">sports_handball</span>&nbsp;<span class="typeName">${gtSvc.getTypeName(groupVO.type_no).type_name}</span>
																</span>
															</h6>
														</div>
														<h5>
						...
						<div class="widget latest-post-widget">
							...
							<div class="share-img-item">
								...
								<div class="content-part">
									<h4><a href="IndividualClassServlet.do?i_class_no=${iClassVO.i_class_no}&action=getOne_For_Display">${iClassVO.c_name}</a></h4>
		...
		$(document).ready(function() {
			...
			$("#scopeArea>li").click(function() {
				if($(this).text()==='CityScope'){
					...
					var city = [ '基隆市', '台北市', '新北市', '桃園市', '新竹市', '新竹縣', '苗栗縣', '台中市',
						'彰化縣', '南投縣', '雲林縣', '嘉義市', '嘉義縣', '台南市', '高雄市', '屏東縣', '台東縣',
						'花蓮縣', '宜蘭縣' ];
					var selectCity = '<li data-value="search" class="option selected focus" value="search" selected>Search by city</li>'
						for (i in city) {
							selectCity +="<a href='/EA102G4/front-end/gpt/group.do?action=getCity_For_Search&cityScope=" + city[i] + "'>" + "<li data-value='search' class='option selected focus' value='" + city[i] +"'>" + city[i] + "</li>" + "</a>";
						}
						$(".nice-select .list").css("height","240px")
						$(".list").html(selectCity);
					}else if($(this).text()==='TypeScope'){
						currentScope = $(this).text();
						var selectType = '<li data-value="search" class="option selected focus" value="search" selected>Search by type</li>'
							for (i in jsonGroupTypeData) {
								selectType += "<a href='/EA102G4/front-end/gpt/group.do?action=getType_For_Search&typeScope=" + jsonGroupTypeData[i].type_no + "'>"+"<li data-value='search' class='option selected focus' value='" + jsonGroupTypeData[i].type_no+"'>" + jsonGroupTypeData[i].type_name + "</li>"+ "</a>";
							}
							$(".nice-select .list").css("height","240px")
