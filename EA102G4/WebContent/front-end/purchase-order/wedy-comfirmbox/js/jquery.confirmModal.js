...
(function ($) {
    ...
    jQuery.confirmModal = function(message, options, callback) {
        ...
        var tftf = null;
        if(poPayment == 10){
        	console.log('poPayment: ' + poPayment);
        	tftf = 'text';
        }else if(poPayment == 20){
        	console.log('poPayment: ' + poPayment);
        	tftf = 'hidden';
        }
                ...
                <div style="max-width: ` + modalBoxWidth + `;" class="modal-dialog ` + modalVerticalCenter + `" role="document">
                    <div class="modal-content">
                        <div style="padding: 0.6rem" class="modal-header">
                            <h4 class="modal-title"  style="font-family:Microsoft JhengHei;">` + messageHeader + `</h4>
                        ...
                        <div style="color:black;padding: 0.8rem; font-size: 0.9rem;" class="modal-body text-break forAppend">` + message + `<input type="` + tftf + `"/>` + (poPayment == 10 ? ` 末三碼:` + `<input type="text" size="3"/>` : ``) + `</div>
                        <div style="padding: 0.55rem; -webkit-touch-callout: none; -webkit-user-select: none; -khtml-user-select: none; -moz-user-select: none; -ms-user-select: none; user-select: none;" class="modal-footer">
                            <button type="button" style="` + (confirmButton === 'OK' ? `width: 66px;` : ``) + ` padding: .18rem .5rem;margin-right:10px; " class="confirmButton btn btn-primary btn-sm">` + confirmButton + `</button>
