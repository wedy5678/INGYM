	$(document).ready(paydel);
	$(document).ready(chooseAble);
	$(document).ready(purchaseOrder);
	$(document).ready(afterErrorQuantity);
	$(document).ready(afterErrorPayDelivery);
	$(document).ready(address);
//	$(document).ready(afterErrorAddress);
	
//	function afterErrorAddress(){
//		let s = $(".afterErrorAddress").val();
//		console.log("測試的: " + s);
//		let ss = null;
//		let address = null;
//		if(s.indexOf("市") > 3){
//			if(s.indexOf("市") != -1){
//				address = s.slice(s.indexOf("市") + 1);
//			}
//		}
//		if(s.indexOf("鄉") != -1){
//			address = s.slice(s.indexOf("鄉") + 1);
//		}
//		if(s.indexOf("鎮") != -1){
//			address = s.slice(s.indexOf("鎮") + 1);
//		}
//		if(s.indexOf("區") != -1){
//			address = s.slice(s.indexOf("區") + 1);
//		}//else if(s.s.indexOf("市") != -1){
//			//address = s.slice(s.indexOf("台") + 1);
//		//}
//		console.log("測試的: " + address);
//		$(".addressPlusAddress3").val(address);
//	}
	...
	function address(){
		var app = window.AddressSeleclList =
		{
	       ...
	       $(".addressPlusAddress1").on('change',function(){
	    	   $(".theFullAddress").val("");
	    	   $(".theFullAddress").val(AddressSeleclList.ReturnSelectAddress('縣市1', '鄉鎮市區1') + $(".addressPlusAddress3").val());
	    	   });
	       $(".addressPlusAddress2").on('change',function(){
	    	   $(".theFullAddress").val("");
	    	   $(".theFullAddress").val(AddressSeleclList.ReturnSelectAddress('縣市1', '鄉鎮市區1') + $(".addressPlusAddress3").val());
	       });
	       $(".addressPlusAddress3").on('change',function(){
	    	   $(".theFullAddress").val("");
	    	   $(".theFullAddress").val(AddressSeleclList.ReturnSelectAddress('縣市1', '鄉鎮市區1') + $(".addressPlusAddress3").val());
	       });
	}
	...
	function afterErrorPayDelivery(){
		var afterErrorPayment = $("#afterErrorPayment").val();
		var afterErrorDelivery = $("#afterErrorDelivery").val();
		console.log("afterErrorPayment: " + afterErrorPayment);
		console.log("afterErrorDelivery: " + afterErrorDelivery);
		
		if(afterErrorPayment != "" && afterErrorPayment != "請選擇付款方式"){
			document.getElementById(afterErrorPayment).selected = "true";
		}
		if(afterErrorDelivery != "" && afterErrorDelivery != "請選擇寄送方式"){
			var pi = parseInt(afterErrorDelivery,10);
			var piplus = pi + 30 ; 
			document.getElementById(piplus.toString()).selected = "true";
		}
		
		var selectPayment = document.getElementById('payment');
//		var paymentTr = document.getElementById('paymentTr');
		var selectDelivery = document.getElementById('delivery');
		var addressTr = document.getElementById('addressTr');
		
		var val = selectPayment.value;
		var val2 = selectDelivery.value;
		
//		if(val == 10){
//			console.log('if=true');
//			paymentTr.style.display='';
//		}else{
//			console.log('if=false' );
//			paymentTr.style.display='none';
//		}
	...
	function afterErrorQuantity(){ 			//錯誤驗證後將值放入購買數量
		var afterErrorQuantity = parseInt($("#afterErrorQuantity").val(),10);
		console.log("#afterErrorQuantity: " + afterErrorQuantity);
		
		if(isNaN(afterErrorQuantity) == false){
			console.log("$('#count').val(afterErrorQuantity);");
			$("#count").val(afterErrorQuantity);
			//起始的總計
	...
	function purchaseOrder(){
		$("#forSeller").on('change',function(){
			$("#forSellerInput").val = $("#forSeller").val();
		});
	}
	
	
	function chooseAble(){ 			//取得付款、配送內的字串比對提供選項
		var poPaymentChoose = $("#poPaymentChoose").val();
		var poDeliveryChoose =  $("#poDeliveryChoose").val();
		
		console.log("poPaymentChoose: " + poPaymentChoose);
		console.log("poDeliveryChoose: " + poDeliveryChoose);
		
		if(poPaymentChoose.indexOf($("#10").val()) != -1){
			$("#10").css('display','') ;
		}
		if(poPaymentChoose.indexOf($("#20").val()) != -1){
			$("#20").css('display','') ;
		}
		if(poPaymentChoose.indexOf($("#30").val()) != -1){
			$("#30").css('display','') ;
		}
		if(poDeliveryChoose.indexOf($("#40").val()) != -1){
			$("#40").css('display','') ;
		}
		if(poDeliveryChoose.indexOf($("#50").val()) != -1){
			$("#50").css('display','') ;
		}
	}
...
//		}
		...
		selectDelivery.addEventListener('change',function(e){
		//DEL
			var num2 = selectDelivery.value;
