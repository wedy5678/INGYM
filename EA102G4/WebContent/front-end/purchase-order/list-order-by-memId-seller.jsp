...
<%
	...
	if (memVOLogin == null) { //if (memVOLogin == null && empVO == null) {
		...
		List<PurchaseOrderVO> list = purchaseOrderService.getOrderBySellerMemIdPurchaseOrderVO(memId);
			...
			<div class="row">
				<div class="col-lg-12">
					<div class="cart-title">
						<h2 style="font-family: Microsoft JhengHei; font-size: 30px; font-weight: 500;">賣家訂單管理</h2>
					</div>
				</div>
			</div>
			<div class="row">
					...
					<div class="single-cart-list">
						<table class="table table-borderless">
							<tbody>
								...
								<c:forEach var="purchaseOrderVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
									<tr>
										<td>
											...
											${purchaseOrderVO.poNo}
										</td>
										<td style="">
											<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/product/product.do">
												<p class="font-semibold">

													<a href="#" style="color: inherit;" onclick=parent3FormSub(this)>${productService.getOneProduct(purchaseOrderVO.getProductNo()).pName} </a>
												</p>
										...
										<td>
											<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/purchase_order/purchaseOrder.do">
												...
												<input type="hidden" name="action" value="get-one-order-for-display-seller">
										...
										<td class="deliverySendedBtn" style="display: none;">
											<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/purchase_order/purchaseOrder.do">
												...
												<input type="hidden" id="pName" value="${productService.getOneProduct(purchaseOrderVO.getProductNo()).pName}">
												<input type="hidden" id="memIdBuyer" value="${purchaseOrderVO.memId}">
											</FORM>
										</td>
										<td class="cancelOrderSendBtn" style="display: none;">
											<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/purchase_order/purchaseOrder.do">
										...
										<td class="trButtonsBelowUpdate" style="display: none;">
											<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/purchase_order/purchaseOrder.do">
										...
										<td class="confirmCancelBtn" style="display: none;">
											<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/purchase_order/purchaseOrder.do">
										...
										<td class="waitForBuyerText" style="color: green; display: none;">-等待買方回應-</td>
